...
% Change markup (vs.\ project V1)
\usepackage[normalem]{ulem}
\newcommand{\removed}[1]{\sout{#1}}
\newcommand{\addedinline}[1]{\colorbox{yellow!35}{#1}}
\newcommand{\added}[1]{%
	\par\vspace{0.3em}\noindent
	\colorbox{yellow!35}{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}{#1}}%
	\par\vspace{0.3em}%
}
\newcommand{\replaced}[2]{\removed{#1}\ \addedinline{#2}}
...
\begin{document}
	...
	{\small\textit{Changes relative to project Version 1: \removed{removed}; \addedinline{added}.}}
	...
	The work covers six \replaced{stocks}{candidate stocks} that are managed based on ICES quantitative assessments (Category~1 stocks) under an annual catch advice framework. \added{An initial screening phase may result in the list being refined.}
{\footnotesize	
\begin{table}[h]
	\centering
	\small
	...
	\begin{itemize}
		\item \added{A week~1 screening summary, benchmark projections at target $F$, and confirmed list of case-study stocks.}
...
\begin{enumerate}
	\item \removed{%
	\textbf{Setup (5 days)}:
	agree assessment procedures, data sources, and the advice rule in force
	for each year and stock; confirm backtest period and deliverables.%
	}\\
	\addedinline{\textbf{Initial screening and benchmark projections (5 days)}:}
	\added{%
	...
	for the \replaced{six stocks}{confirmed case-study stocks}, including basic diagnostics.
